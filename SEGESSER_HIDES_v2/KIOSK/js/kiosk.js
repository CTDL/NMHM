...
//setup timer
var time = 1000;
window.setTimeout(goHomescreen, time);
...
/*Disable pinch zoom click in chrome*/
...
var inactivityTime = function () {
    ...
    function resetTimer() {
        clearTimeout(t);
        t = setTimeout(goHomescreen, 20000) //20 seconds
        // 1000 milisec = 1 sec
...
viewer.addHandler('open', function() {
	...
	if (viewer.source.id == 0){
		...
		viewer.viewport.minZoomLevel = 2;	
		minZoom = 2;
		...
		if (isTourEnabled == true){
			...
			var point = new OpenSeadragon.Point(0.25, 0.19);
		...
		viewer.viewport.minZoomLevel = 2.5;
		minZoom = 2.5;
		...
		if (isTourEnabled == true){
			...
			tourSourceLoaded = false;	
		 	...
		 	var point = new OpenSeadragon.Point(0.20, 0.155);
		 	viewer.viewport.panTo(point, true);		
			viewer.viewport.zoomTo(minZoom);
